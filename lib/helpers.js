...
/**
 * matchOpts can be an array of possible matches or an object. The end result should
 * be an array formatted as this: [ { "match": { "name": "elmongoose" } } ]
 * @param matchOpts {Array||Object}
 * @returns {Array}
 */
exports.mustMatchQuery = function _mustMatch(matchOpts) {
  const match = [];

  if (matchOpts) {
    if (Array.isArray(matchOpts)) {
      matchOpts.forEach((matchOpt) => {
        match.push({ match: matchOpt });
      });
    } else {
      match.push({ match: matchOpts });
    }
  }
...
exports.buildNotMatchingQuery = function _buildNotMatchingQuery(mustNotOpts) {
  ...
  if (Array.isArray(mustNotOpts)) {
    mustNotOpts.forEach((opt) => {
      body = {};
      body[opt.type] = opt.expression;
      queries.push(body);
    });
  } else {
    body = {};
    body[mustNotOpts.type] = mustNotOpts.expression;
    queries.push(body);
  }
 ...
 * @return {Object}
 */
exports.buildRangeFilter = function _buildRangeFilter(rangeOpts) {
  const filters = [];

  if (Array.isArray(rangeOpts)) {
    rangeOpts.forEach((rangeOpt) => {
      const rangeFilter = {
        range: {},
      };
      rangeFilter[rangeOpt.key] = rangeOpt.expression;
      filters.push(rangeFilter);
    });
  } else {
    filters.push({
      range: {
        [rangeOpts.key]: rangeOpts.expression,
      },
    });
  }
...
exports.mergeSearchBody = function (searchOpts) {
  ...
  if (searchOpts.mustMatch && Object.keys(searchOpts.mustMatch).length) {
    mustFilters = mustFilters.concat(exports.mustMatchQuery(searchOpts.mustMatch));
  }
...
function createFuzzyBody(fuzzyOpts) {
  const hasFuzziness = !!fuzzyOpts.fuzziness;

  if (!hasFuzziness) {
    throw new Error('Fuzzy Query Exception: fuzziness is missing');
  }
  ...
  opts.query.fuzzy[fuzzyOpts.field] = {
    value: fuzzyOpts.value,
    fuzziness: fuzzyOpts.fuzziness || 2,
    boost: fuzzyOpts.boost || 1.0,
  };

  return opts;
}

exports.doFuzzy = function _doFuzzy(searchUri, fuzzyOpts, cb, options) {
  const fuzzyBody = createFuzzyBody(fuzzyOpts);
  const reqOpts = {
    method: 'POST',
    url: searchUri,
    body: JSON.stringify(fuzzyBody),
  };
  ...
  exports.backOffRequest(reqOpts, (err, res, body) => {
    ...
    logger.log('info', 'fuzzy results', body);
