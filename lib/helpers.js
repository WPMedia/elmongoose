...
exports.buildTermFilters = function (termOpts) {
  ...
  for (var i = 0, keys = Object.keys(termOpts); i < keys.length; i++) {
    ...
    if (Array.isArray(termOpt)) {
      ...
      // termsFilter.terms[key] = termOpt
      // filters.push(termsFilter)
      for (var j = 0; j < termOpt.length; j++) {
...
exports.buildAllMatchQuery = function() {
  var queries = [];
  var body = {
    'query': {
      'match_all': {}
...
exports.buildMatchPhraseQuery = function(opts) {
  var queries = [];
  if (opts.length) {
